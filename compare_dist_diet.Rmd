...
thermal_niche_overlap_map<-ggplot()+
  ...
  scale_color_npg()+
  facet_wrap(~year)+
  labs(title="",x="Eastings",y="Northings",col="Thermal Niche\nOverlap")+
...
pesc_knots_summ <- pesc_grd_long %>% 
  # calculate means by year
  group_by(year) %>% 
  summarise(mean_pesc=mean(log_pesc,na.rm=T)) %>% 
  ungroup() 
...
recruitment_plot <- combined_df %>% 
  ...
  # weird fudging to get the secondary axis to look right
  geom_line(aes(year,pesc*4438/121900-500),col='seagreen')+
  geom_pointrange(aes(year,pesc*4438/121900-500,ymax=upper*4438/121900-500,ymin=lower*4438/121900-500),col='seagreen')+
  scale_y_continuous(limits=c(0,4800),sec.axis=sec_axis(~(.+500)*27.467,name="Total PESC"))+
  scale_x_continuous(breaks=seq(1980,2020,by=5))+
