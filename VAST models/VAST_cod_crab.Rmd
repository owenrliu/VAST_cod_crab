...
TmbData = make_data("Version"=Version, 
                    ...
                    "Aniso"=Aniso,
                    "Options"=Options)
save(TmbData,file=here::here('data','VAST output','TMBData.Rdata'))
...
# Optimize
# Bias Correct?
# Changed Bias correct toF 5/12/21-- too much memory required
BiasCorr= FALSE
...
### Diagnostic plots
Edit 05.13.21- this is not working.
```{r}
...
# plot.new()
# Q = plot_quantile_diagnostic(TmbData=TmbData, Report=Report, FileName_PP="Posterior_Predictive",FileName_Phist="Posterior_Predictive-Histogram",
...
TmbData$n_x <- 300
res <- plot_quantile_residuals(Lat_i=dat[,'Lat'], Lon_i=dat[,'Lon'], spatial_list=Spatial_List,TmbData=TmbData, Report=Save$Report, Q=Q, 
...
plot_anisotropy( Obj= Obj, FileName=paste0(fp,"Aniso.png"), Report=Report, TmbData=TmbData )
...
```{r}
#### View Model Outputs ####
source(here('data','scripts','output__helper_fxns.R'))
...
factors <- plot_factors(fit=Save, Report=Save$Report, ParHat=Save$ParHat, Data=TmbData, SD=SDr, mapdetails_list=MapDetails_List, 
                         Year_Set=Year_Set, category_names=levels(dat$spp), plotdir=fp )
fct_loadings <- plot_fct_loadings(factors,dat,rotated = TRUE,fp)

outs <- list(factors=factors,fct_loadings=fct_loadings,Index=Index,range_index=range_index)
